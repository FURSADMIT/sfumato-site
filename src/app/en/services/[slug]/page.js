...
import { SERVICES_EN, getServiceEn } from "@/data/services.en";
...
export async function generateMetadata({ params }) {
  ...
  return {
    title: service ? `${service.name} — sfumàto` : "sfumàto",
    description: service?.short,
    alternates: {
      canonical: `/en/services/${slug}`,
      languages: { ru: `/services/${slug}`, en: `/en/services/${slug}` },
    },
  };
}
...
export default async function ServicePageEn({ params }) {
  ...
  return (
    <main>
      <Header solid allServices backHref="/en/#services" lang="en" />
